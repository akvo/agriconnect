...
def login_user(login_data: LoginRequest,
    ...
    user = UserService.authenticate_user(db, login_data.email,
                                         login_data.password)
...
def update_profile(update_data: SelfUpdateRequest,
...
@router.get("/verify-invitation/{invitation_token}",
...
def verify_invitation(invitation_token: str, db: Session = Depends(get_db)):
    ...
    return InvitationStatusResponse(valid=True,
                                    expired=False,
                                    user_info={
                                        "email": user.email,
...
def accept_invitation(request: AcceptInvitationRequest,
                      response: Response,
                      db: Session = Depends(get_db)):
    """Accept invitation and set password, then login user"""
    # Accept invitation and activate account
    user = UserService.accept_invitation(db, request.invitation_token,
                                         request.password)
