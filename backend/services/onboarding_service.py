...
Birth year must be between 1900 and {current_year}."""
    ...
    def find_lowest_administrative_location(
        ...
        """
        Find matching locations at lowest administrative level using fuzzy matching.

        Dynamically determines the lowest level from admin_level_order config.
        ...
        logger.info(
            f"[OnboardingService] Found {len(candidates)} matching {lowest_level}s "
    ...
    async def _process_field_value(
        ...
        - location: Extract → Fuzzy match → Fallback to hierarchical if no match
        ...
        # It uses the normal extraction flow which calls _handle_location_field().
        # If fuzzy match fails, _handle_location_field() starts hierarchical
