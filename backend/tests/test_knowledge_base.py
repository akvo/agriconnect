...
class TestKnowledgeBaseEndpoints:
    ...
    def test_update_knowledge_base_partial(
        self,
        client: TestClient,
        auth_headers_factory,
        db_session: Session,
        ...
        payload = {"title": "Partial Updated"}
        response = client.put(
            f"/api/kb/{kb.id}", json=payload, headers=headers
        )
        assert response.status_code == 200
        assert response.json()["title"] == "Partial Updated"
