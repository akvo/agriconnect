...
class TestCustomerEndpoints:
    ...
    def test_update_customer(self, client: TestClient, db_session: Session):
        ...
        update_data = {"full_name": "Updated Name", "language": "sw"}

        response = client.put(
            f"/api/customers/{customer.id}", json=update_data, headers=headers
        )
    ...
    def test_update_customer_not_found(
        ...
        response = client.put(
            "/api/customers/999", json=update_data, headers=headers
        )
    ...
    def test_get_customer_by_phone(
        ...
        response = client.get(
            "/api/customers/phone/+255123456789", headers=headers
        )
    ...
    def test_get_customer_by_phone_not_found(
        self, client: TestClient, db_session: Session
    ):
        admin = User(
            email="admin@test.com",
        ...
        response = client.get(
            "/api/customers/phone/+255999999999", headers=headers
        )
    ...
    def test_create_customer(self, client: TestClient, db_session: Session):
        ...
        response = client.post(
            "/api/customers/", json=customer_data, headers=headers
        )
    ...
    def test_create_customer_minimal_data(
        self, client: TestClient, db_session: Session
    ):
        admin = User(
            email="admin@test.com",
        ...
        customer_data = {"phone_number": "+255987654321"}
    ...
    def test_create_customer_duplicate_phone(
        ...
        existing_customer = Customer(
            phone_number="+255123456789", full_name="Existing Customer"
        )
    ...
    def test_customer_endpoints_require_authentication(
        ...
        for method, endpoint in endpoints:
            if method == "GET":
                ...
                response = client.post(
                    endpoint, json={"phone_number": "+255999888777"}
                )
            elif method == "DELETE":
    ...
    def test_delete_customer(self, client: TestClient, db_session: Session):
        ...
        response = client.delete(
            f"/api/customers/{customer.id}", headers=headers
        )

        assert response.status_code == 200
        data = response.json()
        assert (
            "Customer and associated messages deleted successfully"
            in data["message"]
        )
    ...
    def test_delete_customer_with_messages(
        self, client: TestClient, db_session: Session
    ):
        ...
        response = client.delete(
            f"/api/customers/{customer_id}", headers=headers
        )
        ...
        deleted_message1 = (
            db_session.query(Message).filter(Message.id == message1_id).first()
        )
        deleted_message2 = (
            db_session.query(Message).filter(Message.id == message2_id).first()
        )
    ...
    def test_delete_customer_forbidden_for_non_admin(
        self, client: TestClient, db_session: Session
    ):
