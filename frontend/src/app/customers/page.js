...
export default function CustomersPage() {
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    if (authLoading || !user || (user.user_type !== "admin" && user.user_type !== "eo")) {
      return;
    }
  ...
  useEffect(() => {
    if (!authLoading && (user?.user_type === "admin" || user?.user_type === "eo")) {
      fetchCustomers();
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-brand flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-secondary-700 font-medium text-lg">Loading customers...</p>
          <p className="text-secondary-500 text-sm mt-2">Please wait while we fetch the customer data</p>
        </div>
      </div>
    );
  }
